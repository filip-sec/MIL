...
5. **Tréning:** Rovnaký loop ako teraz – `opt.zero_grad()`, `loss.backward()`, `opt.step()`; na validáciu volaj model bez `loss_fn`/`label` a ber `results_dict["logits"]`.
...
## 9. Spustenie MIL tréningu na MetaCentrum (na features)
...
# Virchow2 features
qsub -v FEAT_DIR=/storage/brno2/home/filipsec/MIL/data/trident_out/panda_virchow2_grandqc_20x_224_ov0/20x_224px_0px_overlap/features_virchow2 scripts/mil_training.pbs

# UNI2 features
qsub -v FEAT_DIR=/storage/brno2/home/filipsec/MIL/data/trident_out/panda_uni_v2_grandqc_20x_256_ov0/20x_256px_0px_overlap/features_uni_v2 scripts/mil_training.pbs

# Viac epoch, iný fold
qsub -v FEAT_DIR=.../features_virchow2,EPOCHS=25,FOLD=1 scripts/mil_training.pbs
```

Conda env je predvolene `trident` (musí obsahovať torch, pandas, scikit-learn, h5py). Ak máš samostatné env pre MIL, nastav `CONDA_ENV=mil`.
...
## 11. How to design / propose MIL algorithms
